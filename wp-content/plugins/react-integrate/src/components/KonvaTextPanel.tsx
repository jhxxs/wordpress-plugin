import type { TextPathConfigType } from "@/store/konva"
import {
  activeShapeAtom,
  selectedIdsAtom,
...
const TextPanel = () => {
  ...
  const [selectedShapeList] = useAtom(selectedShapeListAtom)
  const [activeShape] = useAtom(activeShapeAtom)
  const [selectedIds, setSelectedIds] = useAtom(selectedIdsAtom)

  function handleChangeText(text: string) {
    console.log("handleChangeText", text)

    const index = shapeList.findIndex(
      (v) => v.type === "textpath" && selectedIds.includes(v.id!)
    )

    console.log("handleChangeText", index)
    ...
    if (index !== -1) {
      ...
      setSelectedIds([shapeList[index].id!])
    } else {
